...
describe('AuthService', () => {
    ...
    describe('registerUserByAdmin and registerEnterprise', () => {
        ...
        it('creates enterprise and returns token', async () => {
            mockUsersService.findByEmail.mockResolvedValue(null);
            mockedBcrypt.hash.mockResolvedValue('h');
            const u = { _id: 'u4', email: 'ent@e.com', password: 'h', toObject: function () { return { _id: this._id, email: this.email, password: this.password }; } };
            mockUsersService.create.mockResolvedValue(u);

            const res = await service.registerEnterprise({ email: 'ent@e.com', password: 'p' } as any);
            expect(res).toHaveProperty('access_token');
            expect(res.user).toMatchObject({ _id: 'u4', email: 'ent@e.com' });
        });
    });
});
